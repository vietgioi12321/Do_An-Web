...
import LogEntryModel from '../models/LogEntry'; // Trỏ đúng về file Model User đã sửa sạch lỗi ở bước trước

const router = express.Router();

// API POST: Thêm mới người dùng
router.post('/add_logentry', async (req: Request, res: Response) => {
  try {
    ...
    const newLogEntryModel = await LogEntryModel.create({
